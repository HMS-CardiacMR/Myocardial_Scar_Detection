...
from Architectures.Ablation.Architecture_spatio_temporal_dual_residual_attention import spatio_temporal_network
...
checkpoint = ModelCheckpoint(filepath=path_to_save_weights, save_best_only=True, monitor="val_accuracy", mode="max", verbose=1)
...
for patient in tqdm(list_of_patients):
    ...
    y_train.append(float(training_data.loc[training_data["Patient_ID"] == patient]["WMA"].values))
...
for patient in tqdm(list_of_patients):
    try:
        ...
        y_validation.append(float(validation_data.loc[validation_data["Patient_ID"] == patient]["WMA"].values))
...
# x_validation = x_validation .transpose(0, 1, 3, 4, 2)
